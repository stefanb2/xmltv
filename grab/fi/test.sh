...
#
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --ahdmegkeja > /dev/null 2>&1
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --version > /dev/null 2>&1
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --description > /dev/null 2>&1
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${script_dir}/test.conf --offset 1 --days 2 --cache  t_fi_cache  > t_fi_1_2.xml --quiet 2>t_fi_1.log
...
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${script_dir}/test.conf --offset 1 --days 1 --cache  t_fi_cache  --output t_fi_1_1.xml  2>t_fi_2.log
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${script_dir}/test.conf --offset 2 --days 1 --cache  t_fi_cache  > t_fi_2_1.xml 2>t_fi_3.log
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${script_dir}/test.conf --offset 1 --days 2 --cache  t_fi_cache  --quiet --output t_fi_4.xml 2>t_fi_4.log
...
#
perl -pe 's/^#(channel\s+(?:4|5|6|7|8|9|10|11|12)\s+.+)/$1/' <${script_dir}/test.conf >${test_dir}/test.conf
perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${test_dir}/test.conf --offset 1 --days 9 --cache  t_fi_cache  >t_fi_full_10.xml --quiet 2>t_fi_full.log
for d in $(seq 1 9); do
    perl -I ${xmltv_lib} ${script_dir}/tv_grab_fi --config-file ${test_dir}/test.conf --offset $d --days 1 --cache  t_fi_cache  >t_fi_single_$d.xml --quiet 2>>t_fi_single.log
done
